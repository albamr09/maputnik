...
import { colorHighlightedLayer, HighlightedLayer } from "../libs/highlight";
...
function buildInspectStyle(
	...
	if (!originalMapStyle || !originalMapStyle.sources) {
		console.warn(
			"buildInspectStyle: originalMapStyle or sources is undefined, returning empty style",
		);
		return {
			version: 8,
			sources: {},
...
> {
	...
	componentDidUpdate() {
		...
		if (!this.props.mapStyle || !this.props.mapStyle.sources) {
			console.warn(
				"componentDidUpdate: mapStyle or sources is undefined, skipping update",
			);
			return;
		}
	...
	componentDidMount() {
		...
		if (!this.props.mapStyle || !this.props.mapStyle.sources) {
			console.warn(
				"componentDidMount: mapStyle or sources is undefined, cannot create map",
				{
		...
		const inspect = new MaplibreInspect({
			...
			buildInspectStyle: (
				...
				if (!this.props.mapStyle || !this.props.mapStyle.sources) {
					console.warn(
						"MaplibreInspect buildInspectStyle: mapStyle or sources is undefined, returning empty style",
					);
					return {
						version: 8,
						sources: {},
