...
const getThemeEffectiveValue = <K extends keyof ThemeEntryProperties>(
	...
	if (typeof fallback === "number")
		return Number(rawValue) as ThemeEntryProperties[K];
	if (typeof fallback === "boolean")
		return Boolean(rawValue) as ThemeEntryProperties[K];
	if (typeof fallback === "string")
		return String(rawValue) as ThemeEntryProperties[K];
...
const checkPropertyExists = (
	...
	if (type == "string" || type == "color") {
		...
		return [
			"all",
			["has", propertyName],
			["!=", getMaplibreProperty(propertyName, "string"), ""],
...
const createThemeEntryExpression = (
	...
	const defaultValue = getDefaultSafeValue(
		defaultProps,
		propertyName,
		fallback,
	);
...
const createOpacityFilter = (
	...
	conditions.push([
		...
		[
			"==",
			["round", ["*", getMaplibreProperty("fillOpacity", "number"), 10]],
			opacityLevel * 10,
		],
	]);
