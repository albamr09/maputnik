...
import { ExtendedStyleSpecification, MapState, ModalStates } from "../store/types";
import style from "../libs/style";
import { useAppDispatch, useAppSelector } from "../store/hooks";
import { setMapState, toggleModal } from "../store/slices/uiSlice";
import hash from "string-hash";
import { setSelectedLayerIndex, setSelectedLayerOriginalId } from "../store/slices/layersSlice";
import { ValidationError } from "maplibre-gl";
import cloneDeep from "lodash.clonedeep";
import { isEqual, unset } from "lodash";
import { setDirtyMapStyle, setMapStyle, setSpec } from "../store/slices/styleSlice";
...
const useStyleEdition = () => {
    ...
    // Helpers
    const setFetchAccessToken = useCallback((url: string, mapStyle: ExtendedStyleSpecification) => {
    ...
    const updateRootSpec = useCallback((spec: any, fieldName: string, newValues: any) => {
        return {
            ...spec,
            $root: {
                ...spec.$root,
    ...
    const getInitialStateFromUrl = useCallback((mapStyle: ExtendedStyleSpecification) => {
    ...
    const onStyleChanged = useCallback((
        ...
        if (mutableStyle.glyphs && typeof mutableStyle.glyphs === "string") {
            mutableStyle.glyphs = setFetchAccessToken(mutableStyle.glyphs, mutableStyle);
        }

        if (mutableStyle.sprite && typeof mutableStyle.sprite === "string") {
            mutableStyle.sprite = setFetchAccessToken(mutableStyle.sprite, mutableStyle);
        }
